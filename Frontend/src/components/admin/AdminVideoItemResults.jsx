...
const AdminVideoItemResults = ({ onSelectVideo }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-start overflow-y-hidden w-full">
            ...
            <div className="flex flex-col items-center space-y-4 w-full">
                {results.map((video, i) => {
                    if (i === 0) {
                        return <AdminVideoItem ref={firstItemRef} key={i} video={video} onClick={onSelectVideo} />;
                    }
                    if (results.length === i + 1) {
                        return <AdminVideoItem ref={lastHistoryRef} key={i} video={video} onClick={onSelectVideo} />;
                    }
                    return (
                        <AdminVideoItem video={video} key={i} onClick={onSelectVideo} />
                    );
                })}
                <p>
                    <button
...
export default AdminVideoItemResults;
